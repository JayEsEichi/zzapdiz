...
public class FundingProjectController {
     ...
     * 펀딩 프로젝트 수정
     **/
    @PutMapping(value = "/funding/update", consumes = {MediaType.APPLICATION_JSON_VALUE, MediaType.MULTIPART_FORM_DATA_VALUE})
    public ResponseEntity<ResponseBody> fundingProjectUpdate(
            HttpServletRequest request,
            @RequestPart(value = "projectUpdateRequestDto", required = false) FundingProjectUpdateRequestDto fundingProjectUpdateRequestDto,
            @RequestPart(value = "thumbnailImage", required = false) MultipartFile thumbnailImage,
            @RequestPart(value = "videoAndImages", required = false) List<MultipartFile> videoAndImages) {
        log.info("펀딩 프로젝트 수정 api : 수정 요청 유저 - {}, 수정될 프로젝트 id - {}, 썸네일 이미지 - {}, 미디어 파일 개수 - {}",
                request, fundingProjectUpdateRequestDto.getProjectId(), thumbnailImage.getOriginalFilename(), videoAndImages.size());
