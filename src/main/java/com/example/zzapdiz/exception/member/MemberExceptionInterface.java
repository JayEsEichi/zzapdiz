...
public interface MemberExceptionInterface {
    ...
    Boolean matchPassword(String password, String passwordRecheck);

    /** 이미 존재한 회원인지 확인 **/
    Boolean alreadyExistMember(String email);

    /** 회원가입 정보가 모두 정상적으로 적혀있는지 확인 **/
    Boolean allRequestDataCheck(MemberSignupRequestDto memberSignupRequestDto);
    ...
    Boolean checkPassword(String password, String existPassword);

    /** 헤더에 발급된 토큰 정보가 유효한지 확인 **/
    Boolean checkHeaderToken(HttpServletRequest request);
